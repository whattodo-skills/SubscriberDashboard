...
import { calculateMood } from './daily-check-in-mood';
...
export async function listForMember(memberId, access = {}) {
  ...
  return { checkins, mood: calculateMood(checkins), values, stacks, archivedStacks, practiceHistory, recentActivity: practiceHistory, pendingLoop, learningResources };
}
export async function saveCheckinForMember(memberId, entry) { const result = await saveCheckin(memberId, entry); return { ...result, mood: calculateMood(result.checkins) }; }
